...
function obtieneVariableSolicitud() {//le mando el deployment id que es x:x:x y el processid (esto ya esta en la URL y lo seteo en cargapaginainiciatarea)
    $.ajax({
        type: 'GET',
        xhrFields: {
            withCredentials: true,
        ...
        success: function (data) {
            console.log(data);
            console.log('ID de Solicitud= ' + data.variables.Idsolicitud);
            //llamo el WS 
            $.getJSON("http://172.26.20.122:8080/TestRestAPP_Digicel/webresources/ActivacionRenovacionSPP/getActivacionSPP",
            { Idsolicitud: data.variables.Idsolicitud },
            mostrarDatos);
...
function obtieneTareasIndex() {
    $.ajax({
        ...
        error: function () {
            //para pruebas offline de formateo
            console.log('pruebas offline Obtiene tareas INDEX ===');
            var json = {
                "status": null,
                "url": null,
                "index": null,
